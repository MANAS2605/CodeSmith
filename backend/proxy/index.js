...
//can also add login auth,preview auth here if needed, but for now we are just proxying the requests to the target server based on the hostname
//just get jwt token from the request header and verify it with the secret key, if valid then proxy the request to the target server, else return 401 unauthorized
