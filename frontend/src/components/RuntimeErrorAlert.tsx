...
export function RuntimeErrorAlert({
  ...
  return (
    <div className="absolute bottom-4 right-4 z-50">
      <div className="w-[400px] max-w-[calc(100vw-2rem)] bg-card border border-border border-l-[3px] border-l-destructive rounded-[6px] shadow-lg overflow-hidden text-foreground">
        {/* Header */}
        <div className="flex items-center justify-between p-3.5 border-b border-border/60 bg-muted/30">
          <div className="flex items-center gap-2">
            ...
            {error.source && (
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-[3px] bg-muted border border-border text-muted-foreground uppercase">
                {error.source}
              </span>
            )}
          </div>
        ...
        <div className="p-3.5 space-y-2.5">
          ...
          {isExpanded && error.stack && (
            <div className="mt-2 pl-5">
              <div className="p-2.5 bg-muted/40 rounded-[4px] border border-border/70 overflow-auto max-h-[180px]">
